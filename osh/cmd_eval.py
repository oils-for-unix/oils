...
class CommandEvaluator(object):
    ...
    def _DoVarDecl(self, node):
        ...
        probe('cmd_eval', '_DoVarDecl_enter')
        # x = 'foo' in Hay blocks
            ...
            if node.rhs is None:
                for i, lhs_val in enumerate(node.lhs):
                    ...
                    self.mem.SetNamed(lval,
                                      ...
                                      scope_e.LocalOnly,
                                      flags=flags)
                probe('cmd_eval', '_DoVarDecl_exit')
                return 0
    ...
    def _DoMutation(self, node):
        ...
        probe('cmd_eval', '_DoMutation_enter')
        ...
        with switch(node.keyword.id) as case2:
            if case2(Id.KW_SetVar):
                ...
                probe('cmd_eval', '_DoMutation_exit')
    ...
    def _DoSimple(self, node, cmd_st):
        ...
        probe('cmd_eval', '_DoSimple_exit')
        return status

    def _DoExpandedAlias(self, node):
        # type: (command.ExpandedAlias) -> int
        probe('cmd_eval', '_DoExpandedAlias_enter')
        ...
        if len(node.more_env):
            with state.ctx_Temp(self.mem):
                self._EvalTempEnv(node.more_env, state.SetExport)
                probe('cmd_eval', '_DoExpandedAlias_exit')
    ...
    def _DoPipeline(self, node, cmd_st):
        ...
        probe('cmd_eval', '_DoPipeline_enter')
        cmd_st.check_errexit = True
        for op in node.ops:
            if op.id != Id.Op_Pipe:
    ...
    def _DoShAssignment(self, node, cmd_st):
        ...
        if self.check_command_sub_status:
            ...
            probe('cmd_eval', '_DoShAssignment_exit')
    ...
    def _DoExpr(self, node):
        ...
        probe('cmd_eval', '_DoExpr_exit')
        return 0

    def _DoControlFlow(self, node):
        # type: (command.ControlFlow) -> int
        probe('cmd_eval', '_DoControlFlow_enter')
            ...
            if (len(str_val.s) == 0 and
                ...
                if mylib.PYTHON:
                    ...
                    if not (min_int <= arg <= max_int):
                        probe('cmd_eval', '_DoControlFlow_exit')
                        e_die(
                            '%r expected a small integer, got %r' %
                            (lexer.TokenVal(keyword), str_val.s),
                            loc.Word(node.arg_word))
    ...
    def _DoAndOr(self, node, cmd_st):
        ...
        probe('cmd_eval', '_DoAndOr_exit')
        return status

    def _DoWhileUntil(self, node):
        # type: (command.WhileUntil) -> int
        probe('cmd_eval', '_DoWhileUntil_enter')
        status = 0
            ...
            while True:
                try:
                    ...
                    if action == flow_e.Break:
                        ...
                        probe('cmd_eval', '_DoWhileUntil_exit')
                        raise
    ...
    def _DoForEach(self, node):
        ...
        probe('cmd_eval', '_DoForEach_enter')
            ...
            with tagswitch(val) as case:
                if case(value_e.List):
                    ...
                    if n == 1:
                        ...
                        probe('cmd_eval', '_DoForEach_exit')
                        e_die_status(
                            2,
    ...
    def _DoForExpr(self, node):
        ...
        probe('cmd_eval', '_DoForExpr_enter')
            ...
            while True:
                ...
                try:
                    ...
                    if action == flow_e.Break:
                        ...
                        probe('cmd_eval', '_DoForExpr_exit')
                        raise
    ...
    def _DoIf(self, node):
        ...
        probe('cmd_eval', '_DoIf_exit')
        return status

    def _DoCase(self, node):
        # type: (command.Case) -> int
        probe('cmd_eval', '_DoCase_enter')
    ...
    def _Dispatch(self, node, cmd_st):
        ...
        probe('cmd_eval', '_Dispatch_enter')
    ...
    def ExecuteAndCatch(self, node, cmd_flags=0):
        ...
        """
        probe('cmd_eval', 'ExecuteAndCatch_enter')
        ...
        try:
            ...
            probe('cmd_eval', 'ExecuteAndCatch_exit')
            raise

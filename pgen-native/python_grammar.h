#ifndef __PYTHON_GRAMMAR_H__
#define __PYTHON_GRAMMAR_H__

#include "grammar.h"

extern grammar python_grammar;


// XXX: This is from graminit.h in the python source. So this is
// another missing piece that we have to generate. I.e. this is the
// mapping from start symbol names to state indices.
//
// As an aside, the output text might be easier to follow if it
// actually *used* these defines instead of just including the
// constants...
//
// Also, if we change these to static const, we can see them in the
// debugger if we ever find ourselves in such a miserable situation as
// to have to be debugging this code.
//
// Somewhat annoyingly, parsetok uses these symbols internally, which
// means certain feature of the python grammar are hard-coded.

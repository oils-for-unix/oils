    ...
    def _ApplyRedirect(self, r):
        ...
        with tagswitch(arg) as case:
            ...
            if case(redirect_arg_e.Path):
                ...
                noclobber = O_EXCL if self.mutable_opts.Get(option_i.noclobber) else 0
                ...
                try:
                    ...
                    if noclobber != 0 and e.errno == EEXIST:
                        self.errfmt.PrintMessageEx("I/O redirect error: can't overwrite existing file %r: %s" %
                                      (arg.filename, pyutil.strerror(e)),
                                      self.mutable_opts.Get(option_i.verbose_errexit),
                                      r.op_loc)
                    else:
                        self.errfmt.PrintMessageEx("I/O redirect error: can't open file %r: %s" %
                                      (arg.filename, pyutil.strerror(e)),
                                      self.mutable_opts.Get(option_i.verbose_errexit),
                                      r.op_loc)
                    raise IOError(0)  # redirect failed, errno=0 to hide error at parent level

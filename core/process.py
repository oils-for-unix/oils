...
class Process(Job):
  ...
  def Start(self, why):
    ...
    if pid < 0:
      ...
      # XXX: If performance is a concern here we may not want to use
      # `signal_def.GetNumber()` in the calls to `pyos.Sigaction()` below, as
      # the code generated by `frontend/signal_gen` isn't super efficient
      # because of the string comparisons.

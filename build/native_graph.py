...
- Rules for build.ninja
  - also in mycpp/build_graph.py
  - delete shell wrappers

- Fold bloaty reports in here?  Depends on the symbolized, optimized file
  ...
  - make bloaty report along with total size in the continuous build?
    ...
    - maybe just output the TSV file first
...
def NinjaGraph(n):
  ...
  n.build(BUILD_NINJA, 'native-graph', ['build/native_graph.py'])
  ...
  for compiler in ['cxx', 'clang']:
    for variant in [
      ...
      n.build(bin_together, 'compile-and-link',
      ...
      for src in sources:
        ...
        n.build(obj, 'compile-one', [src], implicit=[BUILD_NINJA],

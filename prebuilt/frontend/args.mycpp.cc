...
NoUnset::NoUnset(BigStr* msg, syntax_asdl::loc_t* location) : ::error::FatalRuntime(1, msg, location) {
}

Expr::Expr(BigStr* msg, syntax_asdl::loc_t* location) : ::error::FatalRuntime(EXPR_STATUS, msg, location) {
}
...
args::_Attributes* ParseMore(flag_spec::_FlagSpecAndMore* spec, args::Reader* arg_r) {
  ...
  while (!arg_r->AtEnd()) {
    ...
    if (maybe_str_equals(arg, S_Bjq)) {
      out->saw_single_dash = true;
      arg_r->Next();
      break;
    }
    if (maybe_str_equals(arg, S_jnE)) {
      arg_r->Next();
      continue;
    }

...
xargs.add_argument('-I',              dest='replace_str', metavar='replace-str', nargs=1)
xargs.add_argument('-i', '--replace', dest='replace_str', metavar='replace-str', nargs='?', const='{}')
xargs.add_argument('-L',                dest='max_lines', metavar='max-lines', nargs=1)
xargs.add_argument('-l', '--max-lines', dest='max_lines', metavar='max-lines', nargs='?', const=1)
xargs.add_argument('-n', '--max-args', metavar='max-args', nargs=1)
xargs.add_argument('-s', '--max-chars', metavar='max-chars', nargs=1)
xargs.add_argument('-P', '--max-procs', metavar='max-procs', nargs=1, default=1)
xargs.add_argument('--process-slot-var', nargs=1, metavar='name')

...
_gen-win32-build() {
  ...
  local bat_name=${2:-_build/$source_name.bat}

  local comment="$bat_name: Generated by build/ninja_main.py"

  ninja-main batch $source_name
}
...
gen-win32-build() {
  ...
  # .cmd uses "delayed variable expansion"?
  local bat_name=${2:-_build/$source_name.bat}

  _gen-win32-build $source_name $bat_name > $bat_name
  echo "  (build/ninja_main.py) -> $bat_name" >&2
}
...
gen-shell-build() {
  ...
  local sh_name
  case $source_name in
    oils_for_unix) sh_name='_build/oils.sh' ;;
    *) sh_name="_build/$source_name.sh" ;;
  esac
...
tarball-manifest() {
  ...
  # 100 files
  ninja-main tarball-manifest $source_name
}
...
app-deps() {
  ...
  ninja-main app-deps $source_name
}

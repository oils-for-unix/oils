...
from typing import List, Dict, Tuple, IO, Union, Optional, Any, cast, TYPE_CHECKING

if TYPE_CHECKING:
    from asdl.asdl_main import Abbrev
...
def _WriteUses(f, schema_ast):
    # type: (IO[bytes], ast.Module) -> None
...
def WriteHeaderFile(schema_ast, ARG_0, ns, pretty_print_methods, out_prefix):
    # type: (ast.Module, str, str, bool, str) -> Dict[str, int]
    guard = ns.upper()
...
def WriteCppFile(schema_ast, ARG_0, ns, abbrev, out_prefix):
    # type: (ast.Module, str, str, Abbrev, str) -> None
...
""" % (out_prefix, ARG_0, out_prefix))
        ...
        if abbrev.mod_entries:
            # This is somewhat hacky, works for frontend/syntax_abbrev.py and
            # prebuilt/frontend/syntax_abbrev.mycpp.h
            part0, part1 = abbrev.module.split('.')
            f.write('#include "prebuilt/%s/%s.mycpp.h"\n' % (part0, part1))

        # e.g. runtime::NewRecord() and TraversalState are from asdl/runtime.py
        # TODO: it would be nice to get rid of this
        f.write('#include "prebuilt/asdl/runtime.mycpp.h"  // from mycpp\n')
...
""" % ns)
        ...
        v3 = MethodDefVisitor(f,
                              abbrev_ns=abbrev.ns,
                              abbrev_mod_entries=abbrev.mod_entries)

...
class TopicHtmlRenderer(object):
    ...
    def Render(self, line):
        ...
        if m:
            ...
            out.PrintUntil(m.start(1))
            out.Print('<a href="%s#%s" class="level2">' %
                      (self.html_page, href))
...
class StrPool(object):
    ...
    def Add(self, s):
        ...
        self.global_strs.append('GLOBAL_STR(%s, %s)' %
                                (var_name, json.dumps(s)))
...
''')
        ...
        for path in argv[2:]:
            ...
            if filename.endswith('.md'):
                ...
                box_nodes = []
                html = oils_doc.HighlightCode(html, None, debug_out=box_nodes)
                ...
                raise RuntimeError('Expected toc-* or chap-*, got %r' %
                                   filename)

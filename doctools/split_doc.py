...
META_RE = re.compile(r'(\S+): [ ]* (.*)', re.VERBOSE)
...
def SplitDocument(default_vals, entry_f, meta_f, content_f, strict=False):
    ...
    if strict and first_line.strip() != '---':
        raise RuntimeError("Document should start with --- (got %r)" %
                           first_line)
...
def Options():
    ...
    p.add_option(
        ...
        default=[],
        help=
        "If the doc's own metadata doesn't define 'name', set it to this value"
    )
    p.add_option('-s',
                 '--strict',
                 dest='strict',

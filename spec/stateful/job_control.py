...
@register()
def bug_esrch_pipeline_with_builtin(sh):
    'ESRCH bug - pipeline with builtin'

    # Also see test/bugs.sh, there was a history|less issue
    ...
    for i in range(n):
        ...
        if True:
            #sh.sendline('echo hi | cat')
            sh.sendline('echo hi | cat | cat | cat')
            sh.expect(r'.*hi.*')
        else:
            sh.sendline('echo hi | tr a-z A-Z')
            sh.expect(r'.*HI.*')

...
class Build(ExpressionVisitor[T], StatementVisitor[None]):
    ...
    def resolve_callee(self, o: CallExpr) -> Optional[util.SymbolPath]:
        ...
        if isinstance(o.callee, NameExpr):
            ...
            if isinstance(o.callee.expr, NameExpr):
                ...
                if is_module:
                    return split_py_name(o.callee.expr.fullname) + (o.callee.name,)
                    ...
                    local_type = None
                    for name, _, t in self.local_vars.get(self.current_func_node, []):
                    ...
                    if local_type:
                        if isinstance(local_type, Instance):
                            return split_py_name(local_type.type.fullname) + (o.callee.name,)

                        elif isinstance(local_type, UnionType):
                            assert len(local_type.items) == 2
                            return split_py_name(local_type.items[0].type.fullname) + (o.callee.expr.name,)
                ...
                if isinstance(t, Instance):
                    ...
                    return split_py_name(t.items[0].type.fullname) + (o.callee.name,)

                elif o.callee.expr and getattr(o.callee.expr, 'fullname', None):
                    return split_py_name(o.callee.expr.fullname) + (o.callee.name,)
    ...
    def visit_for_stmt(self, o: 'mypy.nodes.ForStmt') -> T:
        cfg = self.current_cfg()
        with pass_state.CfgLoopContext(cfg, entry=self.current_statement_id) as loop:
            self.accept(o.expr)
    ...
    def visit_func_def(self, o: 'mypy.nodes.FuncDef') -> T:
        ...
        if self.current_class_name and self.virtual.IsVirtual(self.current_class_name, o.name):
            key = (self.current_class_name, o.name)
            base = self.virtual.virtuals[key]
            if base:
                sub = join_name(self.current_class_name + (o.name,), delim='.')
    ...
    def visit_assignment_stmt(self, o: 'mypy.nodes.AssignmentStmt') -> T:
        ...
        if isinstance(o.rvalue, CallExpr):
            ...
            if full_callee and cfg:
                cfg.AddFact(self.current_statement_id,
                            pass_state.FunctionCall(join_name(full_callee,
                                                              delim='.')))

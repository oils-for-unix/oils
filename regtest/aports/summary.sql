...
select
  printf(
    '<tr> <td><b>Elapsed Hours</b></td> <td class="num"><b>%.1f</b></td> </tr>',
    sum(elapsed_minutes) / 60
  )
from metrics;
select
  printf(
    '<tr> <td><b>Notable Disagreements</b></td> <td class="num"><b>%d</b></td> </tr>',
    count(*)
  )
from notable_disagree;
-- Tasks are redundant with APKBUILD files
-- select printf('<tr>Tasks: %d</tr>', sum(num_tasks)) from metrics;
select
  printf(
    '<tr> <td>Unique causes</td> <td class="num">%d</td> </tr>',
...
select
  printf(
    '<tr> <td>Packages without a cause assigned (unknown)</td> <td class="num">%s</td> </tr>',
    count(*)
  )
from notable_disagree
where cause = 'unknown';

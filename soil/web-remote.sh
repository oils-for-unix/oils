...
make-job-wwz() {
  ...
  zip -q -r $wwz \
    index.html \
    _test \
    _tmp/{soil,spec,wild-www,stateful,process-table,syscall,benchmark-data,metrics,mycpp-examples,compute,gc,vm-baseline,osh-runtime,osh-parser,host-id,shell-id} \

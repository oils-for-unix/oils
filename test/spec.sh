...
oil-builtin-run() {
  sh-spec spec/oil-builtin-run.test.sh --osh-failures-allowed 0 \
    $OSH_LIST "$@"
}
...
oil-expr() {
  sh-spec spec/oil-expr.test.sh --osh-failures-allowed 7 \
    $OSH_LIST "$@"
}
...
oil-proc() {
  sh-spec spec/oil-proc.test.sh --osh-failures-allowed 2 \
    $OSH_LIST "$@"
}
...
oil-xtrace() {
  sh-spec spec/oil-xtrace.test.sh --osh-failures-allowed 6 \
    $OSH_LIST "$@"
}
